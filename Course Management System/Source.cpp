...
void add_stu()
{
	...
	while (true)
	{
		...
		cin >> stu.Sid;
		while (infile >> temps.Sid >> temps.Fname >> temps.Lname >> temps.age >> temps.Address)
		{
			if (stu.Sid == temps.Sid)
			{
	...
	while (true)
	{
		...
		if (f2.is_open() && infile.is_open())
		{
			...
			if (found == 0)
			{
				...
				int rchoice;
				cout << "Would You Like To Search Another Student ?" << endl;
	...
	if (f1.is_open())
	{
		while (f1 >> crs.Cid >> crs.CourseName >> crs.InstName >> crs.nomstus)
		{
			if (id == crs.Cid)
			{
				...
				cout << "3--Back to Editing." << endl << "4--Back To Courses Menu" << endl;
				cout << "Choice ==> ";
				cin >> choice;
				switch (choice)
				{
...
void del_stuncrs(string Cid)
{
	ofstream outfile;
	ifstream infile;
	outfile.open("Tempsc.txt");
	infile.open("Courses Data.txt");
	while (infile >> tempc.Cid >> tempc.CourseName >> tempc.InstName >> tempc.nomstus)
	{
		if (Cid == tempc.Cid)
		{
			outfile << tempc.Cid << "\t" << tempc.CourseName << "\t" << tempc.InstName << "\t" << tempc.nomstus - 1 << endl;
		}
		else
		{
			outfile << tempc.Cid << "\t" << tempc.CourseName << "\t" << tempc.InstName << "\t" << tempc.nomstus << endl;
		}
	}
	outfile.close();
	infile.close();
	remove("Courses Data.txt");
	rename("Tempsc.txt", "Courses Data.txt");
}
